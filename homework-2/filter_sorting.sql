...
SELECT AVG(shipped_date - order_date) AS avg_delivery_time FROM orders WHERE ship_country = 'Germany';
...
SELECT MIN(unit_price) AS min_price, MAX(unit_price) AS max_price FROM products WHERE units_in_stock >= 20 AND discontinued <> 1;
